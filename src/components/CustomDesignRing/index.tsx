 ...
 */
const CustomDesignRing = forwardRef<CustomDesignRingRef, {
  beads: Bead[];
  canvasId?: string;
  size?: number;
  spacing?: number;
  ...
  useImperativeHandle(ref, () => ({
    ...
    getSelectedBeadIndex: () => selectedBeadIndex,
  }), [imageUrl, dots, predictedLength, selectedBeadIndex]);
  ...
  const processImages = useCallback(async (_beads: Bead[]) => {
    ...
    try {
      ...
      // 缓存结果
      imageProcessCacheRef.current.set(
        cacheKey,
        JSON.stringify(beadsWithImageData)
      );
  ...
  // 优化：位置计算缓存
  const computeBeadPositions = useCallback(
    (beads: Bead[], spacing: number) => {
      const cacheKey = `${JSON.stringify(
        beads.map((b) => ({ d: b.render_diameter, id: b.id }))
      )}_${spacing}_${canvasSize}`;
  ...
  // 优化：异步处理珠子，避免阻塞UI
  const processBeads = useCallback(
    async (_beads: Bead[]) => {
    ...
    () =>
      debounce(async (dotList: any[], selectedBeadIndex: number) => {
        ...
        try {
          ...
          // 批量绘制非选中的珠子
          const normalBeads = dotList.filter(
            (_, index) => index !== selectedBeadIndex
          );
          const selectedBead =
            selectedBeadIndex !== -1 ? dotList[selectedBeadIndex] : null;
